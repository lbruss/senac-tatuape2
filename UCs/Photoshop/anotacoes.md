...
Ela cria um **caminho vetorial**, que funciona como um contorno desenhado ao redor do objeto.
...
É ideal para recortar objetos com bordas bem definidas, como:
...
As ferramentas automáticas tentam adivinhar onde cortar.

A Caneta é como desenhar exatamente por onde a tesoura deve passar.

Por isso, o resultado costuma ser muito mais limpo.
...
começo a contornar o objeto.

Cada clique cria um **ponto de ancoragem**.

O Photoshop liga esses pontos formando um caminho.
...
Não há problema em gastar mais tempo nessa etapa.

Na prática profissional, a precisão é mais importante do que a velocidade.
...
Ele é convertido em uma seleção ativa.

O Photoshop exibe a conhecida linha pontilhada ("formigas marchando"), indicando a área selecionada.
...
Assim, qualquer alteração será aplicada apenas ao fundo, enquanto o objeto permanece protegido.
...
* aplicar filtros;
* desfocar o fundo;
* ajustar brilho, contraste ou saturação.

Como o objeto continua fora da seleção, ele permanece intacto durante toda a edição.
...
**Primeiro cria-se o caminho; depois ele é convertido em seleção para realizar a edição.**
...
* o recorte precisa ser extremamente preciso;
* as ferramentas automáticas não conseguem identificar corretamente as bordas;
* o objeto possui contornos bem definidos;
* o trabalho será utilizado profissionalmente.

Embora exija mais prática, ela costuma produzir os melhores resultados em recortes complexos.
...
* Ajuste as curvas clicando e arrastando os pontos de ancoragem.
* Se possível, preserve a imagem original e faça as edições em novas camadas.
* A Caneta exige prática, mas é considerada uma das ferramentas mais precisas do Photoshop para recortes profissionais.
...
3. Clicar e arrastar permite criar curvas suaves.
4. O caminho deve contornar todo o objeto com precisão.
5. Após finalizar, clicar com o botão direito e escolher **Criar Seleção**.
6. A seleção ativa é indicada pela linha pontilhada.
7. `Ctrl + Shift + I` inverte a seleção.
8. Após inverter, apenas o fundo ficará selecionado.
9. O fundo pode ser apagado, substituído ou receber efeitos sem alterar o objeto.
10. A Ferramenta Caneta é uma das técnicas mais precisas para recortes profissionais no Photoshop.
