...
/*18/09/2026*/
/*Ver a quantidade total de produtos vendidos no dia atual(3 jeitos)*/

/*Jeito 1*/
select vendas.dataVenda, itens_venda.quantidade, sum(quantidade)
from itens_venda
inner join vendas
on vendas.idVenda = itens_venda.idVenda;

/*Jeito 2*/
select sum(itens_venda.quantidade) as total_geral_itens_vendidos
from itens_venda
inner join vendas
on vendas.idVenda = itens_venda.idVenda
...
/*Jeito 3*/
select vendas.dataVenda, itens_venda.idProduto, sum(itens_venda.quantidade) as total_vendido
from itens_venda
inner join vendas
on vendas.idVenda = itens_venda.idVenda
where vendas.dataVenda like '2026-09-16%'
